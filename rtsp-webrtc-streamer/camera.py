...
class CameraSource:
    ...
    def __init__(
        ...
        if self.source_url:
            ...
            self.source_label = self.source_url
        else:
            system = platform.system()
            if system == "Windows":
            ...
            self.source_label = f"camera index {camera_index}"
        self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        if not self.cap.isOpened():
            raise RuntimeError(f"Cannot open camera source {self.source_label}")

        if self.source_url:
            self.capture_width = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH) or 0)
            self.capture_height = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT) or 0)
            self.capture_fps = float(self.cap.get(cv2.CAP_PROP_FPS) or fps)
        else:
        ...
        log.info(
            ...
            self.source_label, self.capture_width, self.capture_height, self.capture_fps,
        )
    ...
    async def stop(self) -> None:
        ...
        if self.cap.isOpened():
            self.cap.release()
            log.info("Camera source released: %s", self.source_label)
